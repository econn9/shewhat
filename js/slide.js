(function() {
    ...
    function slideIt(emoji) {
        ...
        if (emoji.hasClass('happy') && !goodState) {
            goodState = !goodState;
            badState = false;
            good.animate({left: '0'}, {duration: 1000, queue: false});
            bad.animate({left: '0'}, {duration: 1000, queue: false});
            good.animate({opacity: '1'}, {duration: 1100, queue: false});
            bad.animate({opacity: '0'}, {duration: 1100, queue: false});
            ...
            badState = !badState;
            goodState = false;
            good.animate({left: '50%'}, {duration: 1000, queue: false});
            bad.animate({left: '50%'}, {duration: 1000, queue: false});
            good.animate({opacity: '0'}, {duration: 1100, queue: false});
            bad.animate({opacity: '1'}, {duration: 1100, queue: false});
            badCTA.css("z-index", z++);
